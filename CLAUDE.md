...
# No local Postgres? Spin one up via docker-compose (postgres:16-alpine, port 5433 by default)
bun run db:up      # start
bun run db:logs    # tail
...
bun run dev
```
- Public site: `http://localhost:5173`, dashboard: `http://localhost:5173/dashboard`
- API health: `http://localhost:3001/health`, OpenAPI docs: `http://localhost:3001/docs` (spec at `/openapi.json`)
...
- `index.ts` registers all routes on one Hono app; auth (`auth.ts`), rate limiting, security headers, and CORS are applied here. It also fires two fire-and-forget startup seeders (`void seedX().catch(...)`, logged not thrown on failure): `bootstrapAdmin.ts`'s `seedBootstrapAdmin` creates/promotes the `BETTER_AUTH_BOOTSTRAP_ADMIN_*` user when `AUTH_MODE=better-auth`, and `seedChecklistItems.ts`'s `seedDefaultChecklistItems` populates default `checklist_items` rows if that table is empty.
